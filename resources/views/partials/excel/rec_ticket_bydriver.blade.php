...
<div class="content">
	<div class="panel panel-default">
		<div class="sty_header">
			<p> {{ $coname }} </p>
			<p> RECEIVE TICKETS REPORT BY DRIVER  </p>
			<p> FROM: {{ $date_from }} </p>
		</div>
	</div>
				...
				<div class="panel">
					<div class="panel-body">
						...
						@foreach( $groupBy as $key => $group )
							...
							//$totalnet_bushel = number_format($group->totalnet / 56 , 2);
							//$totalnetdrywt_bushel = number_format($group->totalnetdrywt / 56, 2);
							...
							@if( ( $commoditie_current !== $commoditie_old ) && ( $totalnetdrywt_commoditie !== 0 && $totalnet_commoditie !== 0 ) )
								<table style="margin-top:0px" class="without_border column-fixed">
									<tr>
										<td class="without_border"> Total {{ isset($commoditie_old) ? $commoditie_old : 'N/A' }}</td>
							...
							if( $commoditie_old === '' || ( $commoditie_old === $commoditie_current )){
								...
								/*$totalnet_commoditie_bushel = number_format($totalnet_c / 56, 2);
                                $totalnetdrywt_commoditie_bushel = number_format($totalnetdrywt_c / 56, 2);*/
							}else{
							...
							<table>
								...
								@foreach($infoFormato as $clave => $field)
									...
									/*$bushel_net = \App\NumberFormat::bushelFormat($field->net);
        							$bushel_netdrywt = \App\NumberFormat::bushelFormat($field->netdrywt);*/
									?>
							...
							<table class="without_border column-fixed">
								<tr>
									<td class="without_border"> Total {{ isset($group->farm_name) ? $group->farm_name : 'N/A' }}</td>
								...
								@if( $longitud == $count_iteration )
									<tr>
										<td class="without_border"> Total {{ isset($group->commoditie_name) ? $group->commoditie_name : 'N/A' }}</td>
										<td class="without_border"> {{ $totalnetdrywt_commoditie }} </td>
									</tr>
								@endif
								@if( $longitud == $count_iteration )
