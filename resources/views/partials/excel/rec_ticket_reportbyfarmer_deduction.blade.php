...
<div class="content">
	<div class="panel panel-default">
		<div class="sty_header">
			<p> {{ $coname }} </p>
			<p> RECEIVING TICKETS REPORT BY FARMER WITH DEDUCTION </p>
			<p> FROM: {{ $date_from }} </p>
		</div>
	</div>
						...
						@foreach( $groupBy as $key => $group )
							...
							@if( ($commoditie_current.$seller_current !== $commoditie_old.$seller_old ) && ( $totalnetdrywt_seller !== 0 && $totalnet_seller !== 0 ) )
								<table style="margin-top:0px" class="without_border">
									<tr>
										<td colspan="4"></td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> Total {{ isset($seller_old) ? $seller_old : 'N/A' }}</td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalnet_seller }} </td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalded_seller }} </td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalnetdrywt_seller }} </td>
									</tr>
								</table>
							@endif
							@if( ( $commoditie_current !== $commoditie_old ) && ( $totalnetdrywt_commoditie !== 0 && $totalnet_commoditie !== 0 ) )
								<table style="margin-top:0px" class="without_border">
									<tr>
										<td colspan="4"></td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> Total {{ isset($commoditie_old) ? $commoditie_old : 'N/A' }}</td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalnet_commoditie }} </td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalded_commoditie }} </td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border" style="width: 13%; text-align: right"> {{ $totalnetdrywt_commoditie }} </td>
									</tr>
								</table>
							@endif
							<?php
							...
							<table class="without_border">
								<tr>
									<td colspan="4"></td>
									<td class="col-lg-4 col-md-4 col-sm-6 without_border"> Total {{ isset($group->farm_name) ? $group->farm_name : 'N/A' }}</td>
									<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalnet }} </td>
									<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ number_format($totalded, $decimals_in_tickets) }} </td>
								...
								@if( $longitud == $count_iteration )
									<tr>
										<td colspan="4"></td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> Total {{ isset($group->seller_name) ? $group->seller_name : 'N/A' }}</td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalnet_seller }} </td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalded_seller }} </td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalnetdrywt_seller }} </td>
									</tr>
								...
								@if( $longitud == $count_iteration )
									<tr>
										<td colspan="4"></td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> Total {{ isset($group->commoditie_name) ? $group->commoditie_name : 'N/A' }}</td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalnet_commoditie }} </td>
										<td class="col-lg-4 col-md-4 col-sm-6 without_border"> {{ $totalded_commoditie }} </td>
