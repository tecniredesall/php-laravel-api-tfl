...
<div class="content">
	<div class="panel panel-default">
		<div class="sty_header">
			<p> {{ $coname }} </p>
			<p> SHIPPING TICKETS REPORT BY SILO </p>
			<p> FROM: {{ $date_from }} </p>
		</div>
	</div>
					...
					<div class="panel-body">
						...
						@foreach( $groupBy as $key => $group )
							...
							@if( ( $location_current !== $location_old ) && ( $totalnetdrywt_location !== 0 && $totalnet_location !== 0 ) )
								<table style="margin-top:0px" class="without_border">
									<tr>
										<td style="" class="without_border"> Total {{ isset($location_old) ? $location_old : 'N/A' }}</td>
								...
								@foreach($infoFormato as $clave => $row)
									...
									@if( ($row->date_end == $group->date_end) && ($row->commoditie_id == $group->commoditie_id) && ($row->location_id == $group->location_id))
										<tr id="fieldRow">
											...
											@endif
											<td style=""> {{ $row->commoditie_name }} </td>
											<td style="text-align: center"> {{ $row->trucklicense }} </td>
											<td style=""> {{ $row->location_name }} </td>
											<td style=""> {{ $row->drivername }} </td>
							...
							<table class="without_border">
								...
								?>
								<tr>
									<td style="" class="without_border"> Total {{ isset($group->date_end) ? $fecha : 'N/A' }}</td>
								...
								@if( $longitud == $count_iteration )
									<tr>
										<td style=""  class="without_border"> Total {{ isset($group->location_name) ? $group->location_name : 'N/A' }}</td>
										<td class="without_border"> {{ $totalnet_location }} </td>
										<td class="without_border"> {{ $totalnetdrywt_location }} </td>
									</tr>
								@endif
